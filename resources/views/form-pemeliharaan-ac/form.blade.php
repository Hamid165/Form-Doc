<style>
    ...
    /* PENGATURAN UKURAN TABEL KANAN (ID AC) */
    .table-kanan {
        width: max-content; /* Lebar otomatis menyesuaikan isi agar garis batas kanan pas */
    }
    .kolom-label-kanan {
        width: 107px; /* Lebar khusus kolom tulisan "ID AC" */
    }
    .input-garis-kanan {
        width: 100px; /* Panjang garis putus-putus untuk tempat mengisi */
    }
...
<div class="a4-wrapper" style="flex-direction: column; align-items: center;">
    <div class="top-nav-container" style="width: 100%; max-width: 273mm; margin-bottom: 20px;">
        <a href="{{ route('form-pemeliharaan-ac.index') }}" class="inline-flex items-center text-sm font-medium text-slate-500 hover:text-blue-600 transition-colors mb-6">
            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            Kembali ke Daftar Formulir Pemeliharaan AC
        </a>
    </div>
    <div class="zoom-container" style="zoom: 1.3; width: 100%; display: flex; justify-content: center;">
        <div class="a4-container" style="max-width: 100%; overflow-x: auto;">
            <form id="ac-form" action="{{ $action }}" method="POST">
                @csrf
            ...
            <table class="header-table">
                <tr>
                    ...
                    <td style="width: 22%;">: {{ $formTemplate->no_dokumen ?? 'FR.SM/TI/015.011/10-2020' }}</td>
                </tr>
                <tr>
                    <td>Tanggal</td>
                ...
                <tr>
                    ...
                    <td rowspan="2" class="title-text">
                        FORMULIR CHECKLIST PEMELIHARAAN AC
                    </td>
                    <td>Versi</td>
            ...
            <div class="info-section">
                ...
                <!-- PENGATURAN LEBAR TABEL KANAN (ID AC) -->
                <table class="small-info-table table-kanan">
                <tr>
                    <td class="kolom-label-kanan" style="border: none;">ID AC</td>
                    <td style="border: none;">: 
                        <div style="display: inline-block; position: relative; width: 150px; {{ isset($method) && $method === 'PUT' ? 'pointer-events:none; opacity: 0.8;' : '' }}">
                            <select id="id_ac" name="id_ac" class="form-input-inline input-garis-kanan ts-align-left" style="appearance: none; cursor: pointer;" required oninvalid="this.setCustomValidity('Bagian ini harus diisi')" onchange="this.setCustomValidity('');" {{ isset($method) && $method === 'PUT' ? 'readonly' : '' }}>
                                <option value="">-- Pilih ID AC --</option>
                                @foreach($masterAcs as $ac)
                                <option value="{{ $ac->id_ac }}" data-lokasi="{{ $ac->lokasi }}" data-sub_lokasi="{{ $ac->sub_lokasi }}" data-jenis="{{ $ac->jenis }}" data-merk="{{ $ac->merk }}" data-kapasitas="{{ $ac->kapasitas }}" data-tahun_pasang="{{ $ac->tahun_pasang }}" {{ old('id_ac', $form->id_ac) == $ac->id_ac ? 'selected' : '' }}>{{ $ac->id_ac }}</option>
                                @endforeach
                            </select>
                        </div>
                    </td>
                    ...
                    <td style="border: none;">: 
                        <div style="display: inline-block; position: relative; width: 150px; {{ isset($method) && $method === 'PUT' ? 'pointer-events:none; opacity: 0.8;' : '' }}">
                            <select id="lokasi" name="lokasi" class="form-input-inline input-garis-kanan ts-align-left" style="appearance: none; cursor: pointer;" required oninvalid="this.setCustomValidity('Bagian ini harus diisi')" onchange="this.setCustomValidity('');" {{ isset($method) && $method === 'PUT' ? 'readonly' : '' }}>
                                ...
                                @foreach($masterAcs as $ac)
                                <option value="{{ $ac->lokasi }}" data-id_ac="{{ $ac->id_ac }}" {{ old('lokasi', $form->lokasi) == $ac->lokasi ? 'selected' : '' }}>{{ $ac->lokasi }}</option>
                                @endforeach
                            </select>
                        </div>
                    </td>
            ...
            <table class="main-table">
                ...
                <tbody id="checklist-body">
                    @php
                        $itemsData = $form->items->keyBy('no')->toArray();
                        $maxItems = max(20, count($itemsData) > 0 ? max(array_keys($itemsData)) : 0);
                    @endphp
            ...
            <!-- Catatan Section -->
            <div style="border: 1px solid black; padding: 6px; margin-top: 12px; min-height: 40px; font-size: 10px;">
                <strong>Catatan :</strong><br>
                <textarea id="catatan" name="catatan" rows="3" class="form-input" style="width: 100%; min-height: 40px; font-size: 10px; resize: vertical; white-space: pre-wrap;" placeholder="Catatan :&#10;1. sub lokasi&#10;2. ...">{{ old('catatan', $form->catatan ?? '') }}</textarea>
            </div>
            ...
            <div style="text-align: right; display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px; align-items: center;">
                <a href="{{ route('form-pemeliharaan-ac.index') }}" class="btn-kembali">Batal</a>
...
<div id="importAcItemsModal" class="fixed inset-0 z-[100] hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 m-4" style="font-family: 'Inter', sans-serif;">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-gray-900 m-0">Import Isi Tabel AC</h3>
            <button type="button" onclick="document.getElementById('importAcItemsModal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 transition-colors bg-transparent border-none cursor-pointer">
        ...
        <div class="mb-5">
            ...
            <a href="{{ route('form-pemeliharaan-ac.template-items') }}" class="inline-flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 hover:underline mb-4" style="text-decoration: none;">
        ...
        <div class="flex justify-end gap-3 mt-6">
            <button type="button" onclick="document.getElementById('importAcItemsModal').classList.add('hidden')" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 h-[42px] rounded-lg text-sm font-semibold transition-all border-none cursor-pointer">Batal</button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof TomSelect !== 'undefined') {
        // --- ID AC <-> LOKASI sync ---
        var tsIdAc = new TomSelect("#id_ac", {
            create: false,
        ...
        var isSyncingAc = false;

        tsIdAc.on('change', function(value) {
            if (isSyncingAc) return;
            isSyncingAc = true;
            if (value) {
                var opt = Array.from(document.getElementById('id_ac').options).find(function(o) { return o.value === value; });
                if (opt) {
                    tsLokasi.setValue(opt.getAttribute('data-lokasi'), true);
                    fillCatatanFromAc(opt);
                }
            } else {
                tsLokasi.setValue('', true);
            }
            setTimeout(function() { isSyncingAc = false; }, 50);
        });

        // === Auto-fill Catatan from selected AC data ===
        function fillCatatanFromAc(opt) {
            var catatan = document.getElementById('catatan');
            if (!catatan) return;
            // Only auto-fill if catatan is empty or was previously auto-filled
            var currentVal = catatan.value.trim();
            var isAutoFilled = currentVal === '' || currentVal.startsWith('Catatan :');
            if (!isAutoFilled) return;

            var subLokasi = opt.getAttribute('data-sub_lokasi') || '';
            var jenis = opt.getAttribute('data-jenis') || '';
            var merk = opt.getAttribute('data-merk') || '';
            var kapasitas = opt.getAttribute('data-kapasitas') || '';
            var tahunPasang = opt.getAttribute('data-tahun_pasang') || '';

            var lines = ['Catatan :'];
            if (subLokasi) lines.push('1. Sub Lokasi : ' + subLokasi);
            if (jenis) lines.push('2. Jenis : ' + jenis);
            if (merk) lines.push('3. Merk : ' + merk);
            if (kapasitas) lines.push('4. Kapasitas : ' + kapasitas);
            if (tahunPasang) lines.push('5. Tahun Pasang : ' + tahunPasang);

            catatan.value = lines.join('\n');
        }

        // Auto-fill on page load if AC is already selected (create mode)
        @if(!isset($method) || $method !== 'PUT')
        (function() {
            var currentAcVal = tsIdAc.getValue();
            if (currentAcVal) {
                var opt = Array.from(document.getElementById('id_ac').options).find(function(o) { return o.value === currentAcVal; });
                if (opt) fillCatatanFromAc(opt);
            }
        })();
        @endif
        ...
        tsLokasi.on('change', function(value) {
            ...
            if (value) {
                ...
                if (opt) tsIdAc.setValue(opt.getAttribute('data-id_ac'), true);
            } else {
                tsIdAc.setValue('', true);
            }
            setTimeout(function() { isSyncingAc = false; }, 50);
        });
...
function addRow() {
    ...
    if (window.initCustomDatePicker) {
        window.initCustomDatePicker(tr.querySelector('.custom-date-picker'));
    }
}
...
function processImportItems(btn) {
    ...
    fetch('{{ route('form-pemeliharaan-ac.parse-excel') }}', {
        method: 'POST',
        body: formData
    })
    ...
    .then(result => {
        if (result.success && result.data && result.data.length > 0) {
            ...
            document.getElementById('importAcItemsModal').classList.add('hidden');
        } else {
