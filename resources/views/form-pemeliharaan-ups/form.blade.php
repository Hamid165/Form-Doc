...
<style>
    ...
    /* PENGATURAN UKURAN TABEL KANAN (Nomor Inventaris) */
    .table-kanan {
        width: max-content;
    }
    .kolom-label-kanan {
...
<div class="a4-wrapper" style="flex-direction: column; align-items: center;">
    <div class="top-nav-container" style="width: 100%; max-width: 273mm; margin-bottom: 20px;">
        <a href="{{ route('form-pemeliharaan-ups.index') }}" class="inline-flex items-center text-sm font-medium text-slate-500 hover:text-blue-600 transition-colors mb-6">
            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            Kembali ke Daftar Formulir Pemeliharaan UPS
        </a>
    </div>
    <div class="zoom-container" style="zoom: 1.3; width: 100%; display: flex; justify-content: center;">
        <div class="a4-container" style="max-width: 100%; overflow-x: auto;">
            <form id="ups-form" action="{{ $action }}" method="POST">
                @csrf
            ...
            <table class="header-table">
                <tr>
                    ...
                    <td style="width: 22%;">: {{ $formTemplate->no_dokumen ?? 'FR.SM/TI/015.004/10-2020' }}</td>
                </tr>
                <tr>
                    <td>Tanggal</td>
                ...
                <tr>
                    ...
                    <td rowspan="2" class="title-text">
                        FORMULIR CHECKLIST PEMELIHARAAN UPS
                    </td>
                    <td>Versi</td>
            ...
            <div class="info-section">
                ...
                <table class="small-info-table table-kiri">
                    <tr>
                        ...
                        <td>: <input type="text" name="no_ref" value="{{ old('no_ref', $form->no_ref) }}" class="form-input-inline input-garis-kiri" placeholder="Contoh: 01 / 10 / 2020" required {{ isset($method) && $method === 'PUT' ? 'readonly style=pointer-events:none;background:#f9f9f9;' : '' }}></td>
                    </tr>
                    <tr>
                        <td>Tanggal</td>
                        <td>: <input type="text" name="tanggal" value="{{ old('tanggal', $form->tanggal) }}" class="form-input-inline input-garis-kiri {{ isset($method) && $method === 'PUT' ? '' : 'custom-date-picker' }}" style="cursor: pointer; {{ isset($method) && $method === 'PUT' ? 'pointer-events:none;background:#f9f9f9;' : '' }}" placeholder="Tanggal" autocomplete="off" required {{ isset($method) && $method === 'PUT' ? 'readonly' : '' }}></td>
                    </tr>
                    <tr>
                        <td>Business Area</td>
                        <td>: 
                            <div style="display: inline-flex; align-items: center; gap: 4px;">
                                <input type="text" id="business_area_input" name="business_area" value="{{ old('business_area', $form->business_area ?: 'B060') }}" class="form-input-inline input-garis-kiri" style="pointer-events: none; background: #f9f9f9; width: 120px;" readonly required>
                ...
                <!-- PENGATURAN LEBAR TABEL KANAN (Nomor Inventaris) -->
                <table class="small-info-table table-kanan">
                <tr>
                    <td class="kolom-label-kanan" style="border: none;">NO. ID</td>
                    <td style="border: none;">: 
                        <div style="display: inline-block; position: relative; width: 150px; {{ isset($method) && $method === 'PUT' ? 'pointer-events:none; opacity: 0.8;' : '' }}">
                            <select id="nomor_inventaris" name="nomor_inventaris" class="form-input-inline input-garis-kanan ts-align-left" style="appearance: none; cursor: pointer;" required {{ isset($method) && $method === 'PUT' ? 'readonly' : '' }}>
                                <option value="">-- Pilih Nomor Inventaris --</option>
                                @foreach($masterUps as $ups)
                                <option value="{{ $ups->nomor_inventaris }}" data-lokasi="{{ $ups->lokasi }}" {{ old('nomor_inventaris', $form->nomor_inventaris) == $ups->nomor_inventaris ? 'selected' : '' }}>{{ $ups->nomor_inventaris }}</option>
                                @endforeach
                            </select>
                        </div>
                    </td>
                ...
                <tr>
                    ...
                    <td style="border: none;">: 
                        <div style="display: inline-block; position: relative; width: 150px; {{ isset($method) && $method === 'PUT' ? 'pointer-events:none; opacity: 0.8;' : '' }}">
                            <select id="lokasi" name="lokasi" class="form-input-inline input-garis-kanan ts-align-left" style="appearance: none; cursor: pointer;" required {{ isset($method) && $method === 'PUT' ? 'readonly' : '' }}>
                                <option value="">-- Pilih Lokasi --</option>
                                @foreach($masterUps as $ups)
                                <option value="{{ $ups->lokasi }}" data-nomor_inventaris="{{ $ups->nomor_inventaris }}" {{ old('lokasi', $form->lokasi) == $ups->lokasi ? 'selected' : '' }}>{{ $ups->lokasi }}</option>
                                @endforeach
                            </select>
                        </div>
                    </td>
            ...
            <div style="display: flex; justify-content: flex-end; align-items: center; margin-bottom: 10px; gap: 10px;">
                ...
                <button type="button" onclick="document.getElementById('importUpsItemsModal').classList.remove('hidden')" class="bg-green-600 hover:bg-green-700 text-white px-3 h-[30px] rounded-lg text-[11px] font-bold transition-all shadow-sm shrink-0 flex items-center gap-2" style="border: none; cursor: pointer;">
                    Import Tabel
                </button>
            </div>
            ...
            <table class="main-table">
                ...
                <tbody id="checklist-body">
                    @php
                        $maxItems = max(20, isset($items) ? count($items) : 0);
                    @endphp
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="items[{{ $i }}][tanggal]" value="{{ $item['tanggal'] ?? '' }}" class="form-input {{ $isLocked ? '' : 'custom-date-picker' }}" data-format="id" autocomplete="off" style="text-align: center; cursor: pointer; {{ $isLocked ? 'pointer-events:none; background:#f9f9f9;' : '' }}" placeholder="Tanggal" {{ $i == 1 ? 'required' : '' }} {{ $isLocked ? 'readonly' : '' }}></td>
                        ...
                        <td style="position: relative;">
                            ...
                            @if (!$isLocked)
                            <button type="button" class="btn-delete-row no-print" title="Hapus Baris">✕</button>
                            @endif
                        </td>
                        @else
            ...
            <div class="footer-section clearfix">
                <div class="signature-box">
                    ...
                    <p style="position: relative;">
                        <select id="mengetahui_nama" name="mengetahui_nama" class="form-input-inline sync-signer" data-type="nama" style="width: 100%; text-align: center; appearance: none; cursor: pointer; text-align-last: center;" required>
            ...
            <div style="text-align: right; display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px; align-items: center;">
                <a href="{{ route('form-pemeliharaan-ups.index') }}" class="btn-kembali">Batal</a>
...
<div id="importUpsItemsModal" class="fixed inset-0 z-[100] hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 m-4" style="font-family: 'Inter', sans-serif;">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-gray-900 m-0">Import Isi Tabel UPS</h3>
            <button type="button" onclick="document.getElementById('importUpsItemsModal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 transition-colors bg-transparent border-none cursor-pointer">✕</button>
        </div>
        
        <div class="mb-5">
            <p class="text-sm text-gray-600 mb-3 mt-0">Silakan upload file Excel berformat .xlsx yang berisi data tabel kegiatan.</p>
            <a href="{{ route('form-pemeliharaan-ups.template-items') }}" class="inline-flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 hover:underline mb-4" style="text-decoration: none;">
                Download Template Excel (XLSX)
            </a>
            <input type="file" id="file_import_items_modal" accept=".xlsx, .xls" class="w-full border rounded p-2 text-sm">
        </div>
        
        <div class="flex justify-end gap-3 mt-6">
            <button type="button" onclick="document.getElementById('importUpsItemsModal').classList.add('hidden')" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 h-[42px] rounded-lg text-sm font-semibold transition-all border-none cursor-pointer">Batal</button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof TomSelect !== 'undefined') {
        var tsNomorInventaris = new TomSelect("#nomor_inventaris", {
            create: false,
        ...
        var isSyncingUps = false;

        tsNomorInventaris.on('change', function(value) {
            if (isSyncingUps) return;
            isSyncingUps = true;
            if (value) {
                var opt = Array.from(document.getElementById('nomor_inventaris').options).find(function(o) { return o.value === value; });
                if (opt) tsLokasi.setValue(opt.getAttribute('data-lokasi'), true);
            } else {
                tsLokasi.setValue('', true);
            }
            setTimeout(function() { isSyncingUps = false; }, 50);
        });
        ...
        tsLokasi.on('change', function(value) {
            ...
            if (value) {
                ...
                if (opt) tsNomorInventaris.setValue(opt.getAttribute('data-nomor_inventaris'), true);
            } else {
                tsNomorInventaris.setValue('', true);
            }
            setTimeout(function() { isSyncingUps = false; }, 50);
        });
        ...
        function syncSigners(source, val) {
            ...
            if (source !== 'nama') tomNama.setValue(nama, true);
            if (source !== 'nipp') tomNipp.setValue(nipp, true);
            if (source !== 'jabatan') tomJabatan.setValue(jabatan, true);
...
document.getElementById('btn-add-row').addEventListener('click', function() {
    ...
    tr.innerHTML = '<td class="text-center">' + currentRowCount + '<input type="hidden" name="items[' + currentRowCount + '][no]" value="' + currentRowCount + '"></td>' +
        ...
        '<button type="button" class="btn-delete-row no-print" title="Hapus Baris">✕</button></td>';
    tbody.appendChild(tr);
    
    // Trigger datepicker registration if exists
    if (window.registerCustomDatePicker) {
        window.registerCustomDatePicker();
    }
});
...
function processImportItems(btn) {
    ...
    fetch('{{ route('form-pemeliharaan-ups.parse-excel') }}', {
        method: 'POST',
        body: formData
    })
        ...
        if (result.success && result.data && result.data.length > 0) {
            ...
            result.data.forEach((item, i) => {
                ...
                if (row) {
                    ...
                    if (inTanggal && item.tanggal) inTanggal.value = item.tanggal;
            ...
            document.getElementById('importUpsItemsModal').classList.add('hidden');
        } else {
            alert(result.message || 'Gagal membaca file atau file kosong.');
        }
    })
    .catch(error => {
