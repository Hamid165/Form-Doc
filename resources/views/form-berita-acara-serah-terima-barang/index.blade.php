...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8" x-data="{
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-[17px] font-bold text-slate-900 uppercase">BERITA ACARA SERAH TERIMA BARANG</h1>
                <p class="text-[13px] text-slate-500 mt-0.5">Daftar semua formulir berita acara serah terima barang</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('form-berita-acara-serah-terima-barang.create') }}" class="inline-flex h-[40px] px-5 bg-[#1d4ed8] hover:bg-blue-800 text-white rounded-lg text-[13px] font-semibold items-center justify-center transition-colors whitespace-nowrap">
                Tambah Formulir
            </a>
        </div>
    </div>
    ...
    <div class="text-center py-14 px-4 border border-dashed border-slate-200 rounded-xl bg-slate-50/50">
        <h3 class="text-[15px] font-bold text-slate-900 mb-1">Belum ada data formulir Berita Acara Serah Terima Barang</h3>
    ...
    <div class="space-y-4" x-ref="list">
        ...
        <div data-searchable="true" x-show="search === '' || $el.innerText.toLowerCase().includes(search.toLowerCase())" class="bg-white rounded-xl border border-slate-200 p-5 flex items-center justify-between hover:border-blue-300 transition-all">
            <div class="grid grid-cols-3 gap-8 flex-1">
                <div>
                    <p class="text-[11px] text-slate-500 font-bold mb-1 uppercase tracking-wider">No. Ref</p>
                    <p class="text-[14px] font-bold text-slate-900">{{ $form->no_ref ?: '-' }}</p>
                </div>
                <div>
                    <p class="text-[11px] text-slate-500 font-bold mb-1 uppercase tracking-wider">Penyerah</p>
                    <p class="text-[14px] font-bold text-slate-900">{{ $form->penyerah_nama ?: '-' }}</p>
                </div>
                <div>
                    <p class="text-[11px] text-slate-500 font-bold mb-1 uppercase tracking-wider">Penerima</p>
                    <p class="text-[14px] font-bold text-slate-900">{{ $form->penerima_nama ?: '-' }}</p>
                </div>
            </div>

            <div class="flex items-center gap-2 ml-6">
                <a href="{{ route('form-berita-acara-serah-terima-barang.show', $form->id) }}" class="text-blue-500 hover:text-blue-600 bg-blue-50 hover:bg-blue-100 h-[38px] w-[38px] flex items-center justify-center rounded-lg transition-colors" title="Lihat">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                </a>
                <a href="{{ route('form-berita-acara-serah-terima-barang.edit', $form->id) }}" class="text-amber-500 hover:text-amber-600 bg-amber-50 hover:bg-amber-100 h-[38px] w-[38px] flex items-center justify-center rounded-lg transition-colors" title="Edit">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                </a>
                <a href="{{ route('form-berita-acara-serah-terima-barang.show', $form->id) }}?print=1" class="text-emerald-500 hover:text-emerald-600 bg-emerald-50 hover:bg-emerald-100 h-[38px] w-[38px] flex items-center justify-center rounded-lg transition-colors" title="Cetak">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                </a>
                <form action="{{ route('form-berita-acara-serah-terima-barang.destroy', $form->id) }}" method="POST" class="inline-block m-0">
                    @csrf @method('DELETE')
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
    ...
    <form action="{{ route('master-berita-acara-serah-terima-barang.store') }}" method="POST">
        @csrf
    ...
    <div class="space-y-3">
        ...
        <div x-data="{ editing: false }" class="bg-white rounded-lg border border-slate-200 p-4 flex items-center justify-between">
            ...
            <div x-show="!editing" class="flex items-center gap-2">
                ...
                <form action="{{ route('master-berita-acara-serah-terima-barang.destroy', $signer->id) }}" method="POST" class="m-0">
                    @csrf @method('DELETE')
            ...
            <form x-show="editing" style="display: none;" action="{{ route('master-berita-acara-serah-terima-barang.update', $signer->id) }}" method="POST" class="w-full flex gap-3 items-center">
                @csrf @method('PUT')
