...
@section('title', 'Monitoring Grounding')
...
<div class="grid grid-cols-1 xl:grid-cols-[1fr,420px] gap-6"><!-- ======================= DAFTAR FORMULIR ======================= -->
<div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 mb-8" x-data="{ 
    ...
    <div class="flex justify-between items-end mb-8">
        <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-amber-100 rounded-xl flex items-center justify-center shadow-sm">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Monitoring Grounding</h1>
                <p class="text-sm text-gray-500 mt-1">Daftar semua formulir monitoring grounding</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('form-monitoring-grounding.create') }}" class="inline-flex items-center justify-center px-4 h-[40px] bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold transition-colors w-auto">
                Tambah Formulir
            </a>
        </div>
    </div>
    ...
    <div class="space-y-2" x-ref="list">
        ...
        <div data-searchable="true" x-show="search === '' || $el.innerText.toLowerCase().includes(search.toLowerCase())" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center gap-4 hover:shadow-md transition-shadow group relative">
            <div class="flex-1 grid grid-cols-12 gap-4 items-center">
                ...
                <div class="col-span-2">
                    <p class="text-xs text-gray-500 font-medium mb-0.5">Bulan</p>
                    <p class="text-sm font-medium text-gray-900">{{ $form->bulan ?: '-' }}</p>
                </div>
                <div class="col-span-3">
                    <p class="text-xs text-gray-500 font-medium mb-0.5">Nama Petugas</p>
                    <p class="text-sm font-medium text-gray-900 truncate" title="{{ $form->nama_petugas }}">{{ $form->nama_petugas ?: '-' }}</p>
                </div>
                <div class="col-span-2">
                    <p class="text-xs text-gray-500 font-medium mb-0.5">Tgl Pelaksanaan</p>
                    <p class="text-sm font-medium text-gray-900">{{ $form->tgl_pelaksanaan ?: '-' }}</p>
                </div>
                <div class="col-span-1">
                    <p class="text-xs text-gray-500 font-medium mb-0.5">Items</p>
                    <p class="text-sm font-medium text-gray-900">{{ $form->items->count() }}</p>
                </div>
                <div class="col-span-2 flex justify-end items-center">
                    <div class="flex items-center gap-2">
                        <a href="{{ route('form-monitoring-grounding.show', $form->id) }}?mode=view" class="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 h-[40px] w-[40px] flex items-center justify-center rounded-lg transition-colors" title="Lihat">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                        </a>
                        <a href="{{ route('form-monitoring-grounding.edit', $form->id) }}" class="text-amber-500 hover:text-amber-700 bg-amber-50 hover:bg-amber-100 h-[40px] w-[40px] flex items-center justify-center rounded-lg transition-colors" title="Edit">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        </a>
                        <button type="button" onclick="directPrint('{{ route('form-monitoring-grounding.show', $form->id) }}')" class="text-emerald-600 hover:text-emerald-900 bg-emerald-50 hover:bg-emerald-100 h-[40px] w-[40px] flex items-center justify-center rounded-lg transition-colors" title="Print">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                        </button>
                        <form action="{{ route('form-monitoring-grounding.destroy', $form->id) }}" method="POST" class="inline-block m-0">
                            @csrf
                            @method('DELETE')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p class="text-gray-900 font-semibold mb-1">Belum ada data formulir monitoring grounding</p>
